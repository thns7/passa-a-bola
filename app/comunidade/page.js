...
export default function HomePage() {
  ...
  const MAX_POST_LENGTH = 150; // limite de caracteres
  ...
  // carrega posts sempre que a rota for exibida
  useEffect(() => {
    if (pathname === "/") {
      loadUser();
      loadPosts();
  ...
  const handleLike = (postId) => {
    const currentUser = JSON.parse(localStorage.getItem("currentUser"));
    if (!currentUser) return; // ninguém logado, não curte
    ...
    const newPosts = posts.map((post) => {
      if (post.id === postId) {
        ...
        const updatedLikedBy = alreadyLiked
          ? post.likedBy.filter((u) => u !== currentUser.name) // remove like
          : [...(post.likedBy || []), currentUser.name]; // adiciona like
            ...
            <div>
              ...
              {posts.map((post) => (
                ...
                >
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation(); // <- impede que o clique suba pro container do post
                      handleLike(post.id);
                    }}
                    className="flex items-center gap-1 focus:outline-none"
                  >
                    {post.likedBy?.includes(JSON.parse(localStorage.getItem("currentUser"))?.name) ? (
